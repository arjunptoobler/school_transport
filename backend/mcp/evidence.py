...
@mcp.tool(name="mcp_fetch_dashcam_video")
def fetch_dashcam_video(vehicle_id: str, timestamp: str, duration_seconds: int = 15) -> dict:
    """Fetch a specific video clip from the bus edge DVR system corresponding to the event timestamp."""
...
@mcp.tool(name="mcp_get_edge_confidence_score")
def get_edge_confidence_score(event_id: str) -> dict:
    """Retrieve the raw Edge AI neural network confidence score for a specific detected event."""
...
@mcp.tool(name="mcp_attach_evidence_package")
def attach_evidence_package(incident_id: str, media_urls: list, confidence_score: float) -> dict:
    """Attach media clips and AI confidence scores as an immutable evidence package to the incident ticket."""
